...
Download Csv File
    [Arguments]        ${strUrl}
    Download    ${strUrl}    target_file=${CURDIR}${/}output${/}Data.csv  
...
Read Csv Data and enter in website
    ${sales_reps}=    Read table from CSV    path=${CURDIR}${/}output${/}Data.csv    header=True
...
Export the Receipt as PDF
    ...
    Html To Pdf    ${sales_results_html}    ${CURDIR}${/}output${/}PdfFiles${/}${orderNumber}.pdf


*** Keywords ***
Attach Robot image with pdf
    [Arguments]    ${orderNumber}
    ${listFiles}=    Create List    ${CURDIR}${/}output${/}Robot_Preview.png            
    Add Files To Pdf    ${listFiles}    ${CURDIR}${/}output${/}PdfFiles${/}${orderNumber}.pdf    append=True  

*** Keywords ***
Save Robot Image
    Wait Until Element Is Visible    id:robot-preview-image    timeout=5
    Screenshot    robot-preview-image    ${CURDIR}${/}output${/}Robot_Preview.png

*** Keywords ***
Create ZIP file of the receipts
    Archive Folder With Zip    ${CURDIR}${/}output${/}PdfFiles    archive_name=${CURDIR}${/}output${/}PdfArchive.zip
